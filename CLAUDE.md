...
- `setup_005_dev_stuff.sh` — prompts for a PostgreSQL major version (default `18`; enter `skip` to skip Postgres entirely), then installs `postgresql@<version>`, `redis`, and `mise`. Appends `mise activate` to `~/.zshrc` (fenced). Starts both services. It then pins a global node with `mise use --global node@lts`. **mise owns the language runtimes** — don't add `node` or `ruby` back to `setup_001`'s brew list. The global pin is what makes that safe: a mise shim resolves a version from the project first and the global config second, and with neither set it falls through to whatever is on `PATH`, so dropping the brew copy without pinning would leave a fresh machine with no node at all.
