...
> `~/.claude/settings.json` as a real file. What that costs is under *Why settings.json
> isn't synced*.
>
...
absolute paths on every run. That is machine- and session-specific mutable state, not
config anyone hand-authors. Syncing it via git made every pull a three-way merge against
noise.

So the file is no longer in this repo at all, and `setup_006` does not link it. Each
machine keeps its own real file and the two are allowed to diverge freely. **`git status`
here should be quiet.**

**What this costs: permissions are no longer version-controlled, reviewable in a diff, or
recoverable from a clone.** Nothing off this machine holds a copy of what it is allowed
to do. If that matters, back the file up somewhere the repo isn't. To carry one
intentional change (a new permission rule, a new hook) to the other machine, copy that
piece by hand — don't re-link the whole file.
...
own — delete those keys when you spot them. Model and effort otherwise diverge cleanly via
the overlay, which this change doesn't affect.
...
#    it must show as a real file.
ls -la ~/.claude/{CLAUDE.md,machine.md,overlay.json,settings.json}

# ...and nothing local is linked. This should print nothing at all.
...
3. In a session, run `/context` — the machine file's content should be present, and the
...
- **The Orca hooks are `$HOME`-relative, and Orca will undo that.** Orca generates them
  with `/Users/<you>/` hardcoded. That no longer risks leaking a username into a public
  repo — the file is local and untracked — but re-running Orca's setup, or letting it
  update itself, still overwrites any hand-edit you made to its hook commands. If you
  rewrite the paths to `"$HOME/..."`, the double quotes are load-bearing: inside single
  quotes the shell won't expand `$HOME` and every hook silently no-ops.
